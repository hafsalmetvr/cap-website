...
app.config(function($interpolateProvider) {
...
}), app.config([ function() {} ]), angular.module("cap.controllers.admin", []).controller("SAQCtrl", [ "$scope", "$element", "$http", "$timeout", "$cookies", "customer", function() {} ]).controller("MentorCtrl", [ "$scope", "$element", "$http", "$timeout", "$cookies", "customer", function($scope, $element, $http) {
    $scope.init = function(mentorId) {
        $scope.mentorId = mentorId, $http.get("/rest/mentor/" + $scope.mentorId, {
            headers: {
                "Content-Type": "application/json"
            }
        }).success(function(data) {
            console.log(data), $scope.mentees = data.mentees;
        }).error(function(data) {
            console.log(data);
        });
    };
} ]).controller("MenteeCtrl", [ "$scope", "$element", "$http", "$timeout", "$cookies", "customer", function() {} ]).controller("AdminCtrl", [ "$scope", "$element", "$http", "$timeout", "$cookies", "customer", function() {} ]).controller("SettingsCtrl", [ "$scope", "$element", "$http", "$timeout", "$cookies", "customer", function($scope, $element, $http) {
    $scope.init = function() {
    ...
    $scope.init = function() {
        ...
        $http.post("/rest/customer", angular.toJson($scope.create)).success(function(data) {
            ...
            $scope.success = !0, $scope.msg = "The account was created and confirmation email has been sent to: " + $scope.create.email) : (console.log("error"), 
    ...
    console.log("dboardctrl"), $scope.paginate_questionairs = function() {
        ...
        console.log("dboardctrl init"), customer.get(function(result) {
            $scope.customer = result, console.log($scope.customer), $http.get("/rest/dashboard", {
                ...
                $scope.mentees = data.mentees, $scope.admins = data.admins;
            }).error(function(data) {
                console.log(data);
            });
        });
