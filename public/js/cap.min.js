...
function save_new_note($scope, $http) {
    if (console.log(validate_note($scope)), validate_note($scope)) if (params = {
        ...
        $http({
            ...
            $scope.msg = "true" == data.status ? "Note saved Successfully" : data.message, $scope.get_notes(), 
            $scope.msg = "", $scope.edit_flag = !1, $scope.show_popup = !1, $scope.clear_current_note();
        }).error(function() {});
    } else {
        var url = "/user/notes/";
        $http({
...
function delete_note(note, $scope, $http) {
    $http.delete("/user/notes/" + note.id).success(function() {
        $scope.get_notes();
    }).error(function(data) {
...
function edit_note(note, $scope) {
    $scope.edit_flag = !0, $scope.msg = !1, $scope.show_popup = !0, $scope.current_note.title = note.title, 
    $scope.current_note.id = note.id, $scope.current_note.description = note.description;
}

function validate_note($scope) {
    return "" == $scope.current_note.title ? ($scope.msg = "Please Enter Title", !1) : !0;
}

function clear_current_note($scope) {
    $scope.current_note.title = "", $scope.current_note.id = "", $scope.current_note.share_with_mentee = !1;
}

function LoginController() {}

function ForgotPasswordController($scope, $element, $http) {
    $scope.init = function(token) {
        $scope.email = "", $scope.old_password = "", $scope.new_password = "", $scope.token_value = token;
    }, $scope.validate_form = function() {
        return "" == $scope.email ? ($scope.msg = "Please enter username", !1) : validateEmail($scope.email) ? ($scope.msg = "", 
        !0) : ($scope.msg = "Please enter a valid email address", !1);
    }, $scope.validate_password = function() {
        return "" == $scope.old_password ? ($scope.msg = "Please enter old password", !1) : "" == $scope.new_password ? ($scope.msg = "Please enter new password", 
        !1) : ($scope.msg = "", !0);
        ...
        console.log($scope.token_value), "" != $scope.new_password && "" != $scope.confirm_password ? (params = {
            ...
            "success" == data.passwordreset ? ($scope.msg = "Password changed successfully", 
            document.location.href = "../../login") : $scope.msg = "Some error occured";
        }).error(function() {})) : $scope.msg = "Please Enter Old password and New Password";
    };
}

function MentorMenteeController($scope, $element, $http) {
    $scope.init = function(mentor_id) {
        $scope.mentor_id = mentor_id, $scope.user_type = "Administrator", $scope.get_mentor_mentees();
        ...
        $http.delete("/user/adminmentor/" + mentee.id).success(function() {
...
function QuestionairController($scope, $element, $http, $timeout, $location, $cookies) {
    $scope.init = function(questionair_id) {
        $scope.user_id = $cookies.user_id, $scope.role_id = $cookies.role_id, $scope.questionair_id = questionair_id, 
        $scope.user_type = 1 == $scope.role_id ? "Administrator" : 2 == $scope.role_id ? "Mentor" : "Mentee", 
        $scope.get_questions();
...
function SettingsController($scope, $element, $http, $timeout, $location, $cookies) {
    $scope.init = function() {
        $scope.user_id = $cookies.user_id, $scope.role_id = $cookies.role_id, $scope.reminder_interval = "", 
        $scope.user_type = 1 == $scope.role_id ? "Administrator" : 2 == $scope.role_id ? "Mentor" : "Mentee";
    }, $scope.validate_form = function() {
        return "" == $scope.reminder_interval ? ($scope.msg = "Please Select Reminder Interval", 
        !1) : !0;
    }, $scope.save_settings = function() {
        $scope.validate_form() && (params = {
            ...
            $scope.msg = "success" == data.status ? "Settings Saved Successfully" : "Some error occured";
        }).error(function() {}));
    }, $scope.show_menu = function() {
        $("#menu").css("display", "block");
    }, $scope.hide_menu = function() {
...
function CreateAccountController($scope, $element, $http, $timeout, $location, $cookies) {
    $scope.init = function() {
        $scope.user_id = $cookies.user_id, $scope.role_id = $cookies.role_id, $scope.account_type = "", 
        $scope.name = "", $scope.title = "", $scope.phone_number = "", $scope.email = "", 
        4 == $scope.role_id ? $scope.user_type = "Administrator" : 5 == $scope.role_id ? $scope.user_type = "Mentor" : 6 == $scope.role_id && ($scope.user_type = "Mentee"), 
        $scope.msg = "";
    }, $scope.validate_form = function() {
        return "" == $scope.account_type ? ($scope.msg = "Please Select Account Type", !1) : "" == $scope.name ? ($scope.msg = "Please Enter name", 
        !1) : "" == $scope.title ? ($scope.msg = "Please enter Title", !1) : "" == $scope.phone_number ? ($scope.msg = "Please Enter phone number", 
        !1) : validateEmail($scope.email) ? !0 : ($scope.msg = "Please enter a valid email address", 
        !1);
        ...
        $scope.validate_form() && (params = {
            ...
            $scope.msg = "true" == data.status ? "Account created Successfully" : data.message;
        }).error(function() {}));
    }, $scope.show_menu = function() {
        $("#menu").css("display", "block");
    }, $scope.hide_menu = function() {
...
function AdminMenteeController($scope, $element, $http) {
    $scope.init = function(user, role, mentee_id) {
        $scope.user_id = user, $scope.role_id = role, $scope.mentee_id = mentee_id, console.log(mentee_id), 
        $scope.show_popup = !1, $scope.user_type = 4 == $scope.role_id ? "Administrator" : 5 == $scope.role_id ? "Mentor" : "Mentee", 
...
function AdminMenteeSAQDetailController($scope, $element, $http) {
    $scope.init = function(user, role, saq_id) {
        $scope.user_id = user, $scope.role_id = role, $scope.saq_id = saq_id, $scope.show_popup = !1, 
        $scope.user_type = 4 == $scope.role_id ? "Administrator" : 5 == $scope.role_id ? "Mentor" : "Mentee", 
...
function MenteeDetailController($scope, $element, $http) {
    $scope.init = function(user, role, mentee_id) {
        $scope.user_id = user, $scope.role_id = role, $scope.show_popup = !1, $scope.ques_page_interval = 5, 
        $scope.notes_page_interval = 5, $scope.shared_page_interval = 5, $scope.mentee_id = mentee_id, 
        $scope.edit_flag = !1, $scope.user_type = 4 == $scope.role_id ? "Administrator" : 5 == $scope.role_id ? "Mentor" : "Mentee", 
        $scope.get_mentee_saq_list(), $scope.get_notes(), $scope.get_mentee_shared_notes(), 
        $scope.current_note = {}, $scope.clear_current_note();
...
function MenteeSAQInterface($scope, $element, $http) {
    $scope.init = function(questionare_id) {
        $scope.saq_id = questionare_id, $scope.get_saq_details(), $scope.continue = !0, 
        $scope.pause = !1, $scope.saq = {
        ...
        params = {
            ...
            data.status ? ($scope.message = "Test " + data.status, $scope.question.answer_type = data.status) : ($scope.question = data.question[0], 
            $scope.answers = data.answer, $scope.question.answer_type = data.answer.length > 0 ? data.answer[0].answerType : "TEXT", 
        ...
        params = {
            ...
            AnswerSubmit: 1
        }, "MULTISELECT" == $scope.question.answer_type && (params.selected_answers = angular.toJson($scope.question.selected_answers)), 
        "ENUM" == $scope.question.answer_type && (params.selected_answers = angular.toJson($scope.answers), 
        params.answers_type = "ENUM"), "TEXT" == $scope.question.answer_type && (params.selected_answers = $scope.question.answer_text, 
        params.answers_type = "TEXT"), "TEXTAREA" == $scope.question.answer_type && (params.selected_answers = $scope.question.answer_textarea, 
        params.answers_type = "TEXTAREA"), "CHECKBOX" == $scope.question.answer_type && (params.selected_answers = angular.toJson($scope.answers), 
        params.answers_type = "CHECKBOX"), "RADIO" == $scope.question.answer_type && (params.selected_answers = $scope.question.radio_choice), 
        "SELECT" == $scope.question.answer_type && (params.selected_answers = $scope.question.answer_choice), 
        $http({
            method: "post",
            url: "/user/test",
            data: $.param(params),
        ...
        $scope.continue = !1, $scope.pause = !0, $http.get("/user/summary/" + $scope.saq_id).success(function(data) {
            $scope.saq = data.data;
        }).error(function(data) {
            console.log(data || "Request failed");
        });
    }, $scope.continue_test = function() {
        $scope.continue = !0, $scope.pause = !1;
    };
}

function SAQDetail($scope, $element, $http) {
    $scope.init = function() {}, $scope.get_saq_details = function() {
        params = {
            mentee_id: mentee.id,
...
}(window, window.angular), function(window, document) {
    ...
    angular.module("mgcrea.ngStrap.affix", [ "mgcrea.ngStrap.helpers.dimensions", "mgcrea.ngStrap.helpers.debounce" ]).provider("$affix", function() {
        ...
        return {
            ...
            link: function(scope, element, attr) {
                ...
                angular.forEach([ "template", "placement", "keyboard", "html", "container", "animation", "duration", "dismissable" ], function(key) {
                    ...
                    attr[key] && attr.$observe(key, function(newValue) {
                        scope[key] = $sce.trustAsHtml(newValue);
                    });
                }), attr.bsAlert && scope.$watch(attr.bsAlert, function(newValue) {
        ...
        this.$get = [ "$modal", function($modal) {
            function AsideFactory(config) {
                var $aside = {}, options = angular.extend({}, defaults, config);
                return $aside = $modal(options);
            }
            return AsideFactory;
        } ];
        ...
        return {
            ...
            link: function(scope, element, attr) {
                ...
                }), attr.bsAside && scope.$watch(attr.bsAside, function(newValue) {
                    angular.isObject(newValue) ? angular.extend(scope, newValue) : scope.content = newValue;
                }, !0);
        ...
        this.$get = [ "$window", "$rootScope", "$compile", "$q", "$templateCache", "$http", "$animate", "$timeout", "$sce", "dimensions", function($window, $rootScope, $compile, $q, $templateCache, $http, $animate, $timeout, $sce) {
            function ModalFactory(config) {
                ...
                return $modal.$promise.then(function(template) {
                    ...
                    var parent;
                    parent = angular.isElement(options.container) ? options.container : options.container ? findElement(options.container) : null;
                    var after = options.container ? null : options.element;
                    modalElement = $modal.$element = modalLinker(scope, function() {}), modalElement.css({
                        display: "block"
                    ...
                    scope.$emit(options.prefixEvent + ".hide.before", $modal), $animate.leave(modalElement, function() {
                        ...
                        options.animation && bodyElement.removeClass(options.prefixClass + "-with-" + options.animation);
        ...
        return {
            ...
            link: function(scope, element, attr) {
                ...
                }), attr.bsModal && scope.$watch(attr.bsModal, function(newValue) {
                    angular.isObject(newValue) ? angular.extend(scope, newValue) : scope.content = newValue;
                }, !0);
        ...
        return {
            ...
            link: function(scope, element, attr, controller) {
                ...
                angular.forEach([ "animation" ], function(key) {
                    angular.isDefined(attr[key]) && (options[key] = attr[key]);
                }), attr.bsTabs && scope.$watch(attr.bsTabs, function(newValue) {
                    scope.panes = newValue;
                }, !0), element.addClass("tabs"), options.animation && element.addClass(options.animation), 
                scope.active = scope.activePane = 0, scope.setActive = function(index) {
...
}(window, document), angular.module("cap.controllers", []).controller("NavCtrl", [ "$scope", function($scope) {
    $scope.showMenu = function() {
        $(".hover-menu-container").addClass("active"), $(".hover-menu").removeClass("hide");
    }, $scope.hideMenu = function() {
        $(".hover-menu-container").removeClass("active"), $(".hover-menu").addClass("hide");
    };
} ]).controller("TestCtrl", [ "$scope", function() {} ]), angular.module("cap.directives", []).directive("ngEnter", [ function() {
    return {
        restrict: "EAC",
...
} ]).directive("saqList", [ function() {
    return {
        restrict: "E",
        templateUrl: "/partials/saq-list",
        link: function() {}
    };
} ]).directive("mentorsList", [ function() {
    return {
        restrict: "E",
        templateUrl: "/partials/mentors-list",
        link: function() {}
    };
} ]).directive("menteesList", [ function() {
    return {
        restrict: "E",
...
} ]), angular.module("cap.services", []).factory("customer", [ "$http", function($http) {
    var self = this;
    return self.customer = null, {
        get: function(cb) {
            return console.log("get customer"), self.customer ? self.customer : void $http.get("/rest/customer/current", {
                headers: {
                    "Content-Type": "application/json"
                }
            }).success(function(data) {
                self.customer = data, cb(null, self.customer);
...
}), angular.module("cap.router", [ "ngRoute" ]).config(function($routeProvider, $locationProvider) {
    $locationProvider.html5Mode(!0);
}), angular.module("cap.filters", []).filter("pluralize", [ "$filter", "pluralizeWords", function($filter, pluralizeWords) {
