...
var app = angular.module("cap", [ "ngAnimate", "ngCookies", "cap.controllers", "cap.controllers.mentor", "cap.filters", "cap.router", "cap.services", "cap.directives" ]);
...
angular.module("cap.controllers.mentor", []).controller("DashboardCtrl", [ "$scope", "$element", "$http", "$timeout", "$window", "$cookies", "customer", "overlay", function($scope, $element, $http, $timeout, $window, $cookies, customer, overlay) {
    $scope.init = function() {
    ...
    $scope.createNote = function() {
        ...
        $http.post("/rest/note/" + note.id, {
            note: note,
            customerId: $scope.menteeId
...
} ]).controller("QuestionnaireCtrl", [ "$scope", "$element", "$http", "$timeout", "$window", "$cookies", function($scope, $element, $http, $timeout, $window, $cookies) {} ]).controller("QuestionnairePageCtrl", [ "$scope", "$element", "$http", "$timeout", "$cookies", "customer", "overlay", "$window", function($scope, $element, $http, $timeout, $cookies, customer, overlay, $window) {
    overlay.message("loading questions...").loading(true).show();
    $scope.continue = function(nextUrl) {
        $window.location.href = nextUrl;
    };
} ]).controller("MentorCtrl", [ "$scope", "$element", "$http", "$timeout", "$window", "$cookies", function($scope, $element, $http, $timeout, $window, $cookies) {} ]);