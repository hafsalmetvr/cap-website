...
class AnswerController extends AbstractRestfulController {
  ...
  */
  ...
  public function create( $data ) {
    ...
    /* get the customerQuestionnaire */
    $data['questionnaireId'] = $e->createQuery( "SELECT q.questionnaireId FROM CAP\Entity\Question q WHERE q.id = :questionId" )
                                 ->setParameter('questionId',$data['questionId'])
                                 ->getSingleScalarResult();

    $cQuestionnaire = $e->getRepository("CAP\Entity\CustomerQuestionnaire")->findOneBy( array('questionnaire' => $data['questionnaireId'],
                                                                                              'customer'      => $data['customerId'] ) );

    $data['customerId'] = $cQuestionnaire->getCustomer()->getId();

    /* update customer_answer */
    $e->transactional(function($e) use ($data, $logger) {
      $now = date("Y-m-d H:i:s");
      $conn = $e->getConnection();
      $sql = "delete ca from customer_answer ca join answer a on a.id = ca.answer_id where ca.customer_id = ".$data['customerId']." and a.question_id = ".$data['questionId'];
      $conn->query($sql);

      /* delete all customer answers for this question id and customer id */
      /*
      ...
      */
      ...
      /* createa row for each answer passed in */
    ...
    /* update statuses for all sections and the questionnaire */
    $e->transactional(function($e) use ($data, $logger) {

      $conn = $e->getConnection();

      /* count of questions that are not complete, by section */
      $sql = "select count(*) as count,cs.name, s.id from customer_question cq join question q on q.id = cq.question_id join completion_status cs on cs.id = cq.completion_status_id join section s on s.id = q.section_id where cq.customer_id = " .$data['customerId'] ." AND q.questionnaire_id = ".$data['questionnaireId']." group by cs.name, s.id";
      ...
      /* set section status */
      $cqr = $e->getRepository("CAP\Entity\CustomerQuestionnaire")->findOneBy( array('questionnaire' => $data['questionnaireId'],
                                                                                     'customer' => $data['customerId'] ) );
    ...
    $qService = $this->getServiceLocator()->get( 'cap_questionnaire_service' );
    $viewArgs['percentComplete'] = $qService->percentComplete($data['questionnaireId'], $data['customerId'], $e);
