...
class ResetpasswordController extends AbstractRestfulController
{
     ...
     * @return Zend\View\Model\ViewModel
     */
    public function create_invalid($data)
    {
        if($user = $this->identity()) {
           # return $this->redirect()->toRoute($this->getOptions()->getLoginRedirectRoute());
        }
       ...
       #     $form->setValidationGroup('csrf', 'captcha');
        #    $form->setData($this->getRequest()->getPost());
            if(2==2) {
                ...
                $entityManager = $this->getEntityManager();
                $user = $entityManager->createQuery("SELECT u FROM CsnUser\Entity\Customer u WHERE u.email = '$data[email]'")->getResult(\Doctrine\ORM\Query::HYDRATE_OBJECT);
                $user = $user[0];
                ...
                if(isset($user)) {
                    try {
                        ...
                        return new JsonModel(array( 'data' => 'sent' ));
                       # $entityManager->persist($user);
                       # $entityManager->flush();
     ...
     * @return Zend\View\Model\ViewModel
     */
    public function confirmEmailChangePasswordAction_test()
    {
        ...
        if($token !== '' && $user = $entityManager->getRepository('CsnUser\Entity\Customer')->findOneBy(array('registrationToken' => $token))) {
         ...
         # $fullLink = $this->getBaseUrl() . $this->url()->fromRoute('user-index', array( 'action' => 'login'));
          #$sent =$this->sendEmail(
           ...
           #   sprintf($this->getTranslatorHelper()->translate('Hello again %s. Your new password is: %s. Please, follow this link %s to log in with your new password.'), $user->getEmail(), $password, $fullLink)
         ...
         # );          
          ...
          $this->HeadScript()->appendFile('/Cap/public/js/angular_app/lib/angular/angular.js','text/javascript'); 
          $viewModel = new ViewModel(array(
              'email' => $email,
              'navMenu' => $this->getOptions()->getNavMenu()
